...
(define (rain is_rain)
    ...
    With heavy downpour expect 50 yards visibility and rough terrain. \n")
            (hurricane (random 4) (+ 1 (random 24))))))

(define (hurricane is_hurricane hour)
    (cond
        ((equal? 1 is_hurricane) 
            (display "The party is hurrased by a natural disaster starting at ")
            (write hour)
            (display ":00 and lasting a few hours. 
    Boats sink in 15 minutes if not covered
    1 level of exhaustion is automatically gained if characters venture into the storm
    DC 10 constitution check or the characters take another each hour of travel
    Disadvantage on survival checks for the day\n\n"))
        (else (display "\n"))))

(define (sand_storm does_it_billow)
    (cond
        ((< does_it_billow 40) 
            (display "Today the winds are still.\n\n"))
        ((> does_it_billow 90) 
            (display "Today a gust blows, hot dry air pelts the party.\n\n"))
        (else 
            (display "The winds kick into high gear today preventing flight and
        making all terain rough. Visibility is limited today. \n")
            (zero_visibility (random 4) (+ 1 (random 24))))))

(define (zero_visibility is_sandstorm hour)
    (cond
        ((equal? 1 is_sandstorm) 
            (display "A massive sandstorm hits the party at ")
            (write hour)
            (display ":00 and lasting a few hours. 
    All characters are now blind.
    1 level of exhaustion is automatically gained if characters venture into the storm
    DC 10 constitution check or the characters take another each hour of travel
    Disadvantage on survival checks for the day
    Disadvantage to hear in the storm.\n\n"))
        (else (display "\n"))))
...
(define (desert_temp)
    (desert_temp2
        (+ 38 (-(random 12) 6)) ; average 38, high 44, low 32 
        (- (-(random 12) 6) 4) ; average -4, high 2, low -10
        ))

(define (desert_temp2 day_high night_low)
    (display "Today the high is ")
    (write day_high)
    (display " degrees Celsius.\n The low tonight is ")
    (write night_low)
    (display " degrees Celsius.\n\n"))
...
(define (encounters_beach encounter)
    (cond 
        ...
        ((or (equal? encounter 13) (equal? encounter 14))
            (allosaurus (+ 1 (random 3))))
        ((or (equal? encounter 15) (equal? encounter 16))
            (dimetrodon (+ 2 (random 6) (random 6))))
        ((and (> encounter 16) (< encounter 22))
            (plesiosaurus))
        ((and (> encounter 21) (< encounter 29))
            (pteranodon (+ 2 (random 6) (random 6))))
        ((and (> encounter 28) (< encounter 32))
            (quetzalcoatlus (+ 2 (random 4))))
        ((and (> encounter 31) (< encounter 38))
            (velociraptor (+ 3 (random 6) (random 6) (random 6))))
        ((and (> encounter 37) (< encounter 41))
            (dragon))
        ((or (equal? encounter 41) (equal? encounter 42)) 
            (emerald_enclave (+ 2 (random 4)) (* 5 (+ 2 (random 6) (random 6)))))
        ((and (> encounter 42) (< encounter 47))
            (dead_explorer (random 20)))
        ((and (> encounter 46) (< encounter 50))
            (flaming_fist (+ 2 (random 6)) (+ 1 (random 4))))
        ((and (> encounter 49) (< encounter 53))
            (flying_monkeys))
        ((and (> encounter 52) (< encounter 56))
            (flying_snakes))
        ((or (equal? encounter 56) (equal? encounter 57))
            (frost_giant (random 5)))
        ((and (> encounter 57) (< encounter 64))
            (giant_lizard (+ 1 (random 6))))
        ((and (> encounter 63) (< encounter 68))
            (giant_snapping_turtle))
        ((and (> encounter 67) (< encounter 72))
            (lizardfolk (+ 2 (random 4) (random 4))))
        ((and (> encounter 71) (< encounter 75))
            (red_wizard (+ 1 (random 6))))
        ((and (> encounter 74) (< encounter 85))
            (sea_hag))
        ((and (> encounter 84) (< encounter 88))
            (stirge (+ 2 (random 6))))
        ((and (> encounter 87) (< encounter 90))
            (swarm_of_bats (+ 1 (random 4))))
        ((and (> encounter 89) (< encounter 95))
            (tabaxi_hunter))   
...
(define (encounters_jungle logic)
  (case logic
    ...
    ((59) (giant_wasp (+ 1 (random 6))))
    ((66) (jaculis (+ 1 (random 6))))
    ((67) (kamadan (+ 1 (random 3))))
    ((72) (traps (+ 2 (random 4))))
    ((73) (night_hag))
    ((74) (pterafolk (+ 3 (random 4))))
    ...
    (else
      (cond
        ...
        ((and (> logic 29) (< logic 35)) (velociraptor (+ 3 (random 6) (random 6) (random 6))))
        ((and (> logic 36) (< logic 42)) (emerald_enclave (+ 2 (random 4)) (* 5 (+ 2 (random 6) (random 6)))))
        ((or (equal? logic 42) (equal? logic 43)) (dead_explorer (random 20)))
        ((and (> logic 45) (< logic 50)) (flaming_fist (+ 2 (random 6)) (+ 1 (random 4))))
        ...
        ((and (> logic 81) (< logic 85)) (statue_ubtao (random 4)))))))
...
(define (encounters_lesser_undead logic)
  (case logic
    ...
    (else
      (cond
        ((and (> logic 1) (< logic 5)) (assassin_vines (+ 1 (random 3))))
        ...
        ((and (> logic 22) (< logic 26)) (flaming_fist (+ 2 (random 6)) (+ 1 (random 4))))
        ...
        ((and (> logic 34) (< logic 39)) (mad_monkey_mist))
        ((or (equal? logic 39) (equal? logic 40)) (traps (+ 2 (random 4))))
        ((or (equal? logic 42) (equal? logic 43)) (pterafolk (+ 3 (random 4))))
        ((or (equal? logic 46) (equal? logic 47)) (snake_constrictor))
        ((or (equal? logic 50) (equal? logic 51)) (spider (+ 1 (random 6))))
        ((and (> logic 51) (< logic 55)) (statue_ubtao (random 4)))
        ((or (equal? logic 55) (equal? logic 56)) (stirge (+ 2 (random 6))))
        ((or (equal? logic 57) (equal? logic 58)) (su-monster (+ 2 (random 4))))
        ((and (> logic 58) (< logic 62)) (swarm_of_bats (+ 1 (random 4))))
        ((and (> logic 61) (< logic 65)) (swarm_of_insects (+ 1 (random 4))))
        ((and (> logic 66) (< logic 72)) (ghoul (+ 2 (random 6) (random 6))))
        ((and (> logic 71) (< logic 77)) (skeleton (+ 3 (random 6) (random 6) (random 6))))
        ((or (equal? logic 77) (equal? logic 78)) (specter))
        ((and (> logic 79) (< logic 89)) (zombie (+ 1 (random 10)) (+ 3 (random 6) (random 6) (random 6)) (+ 1 (random 4))))
...
(define (encounters_greater_undead logic)
  (case logic
    ...
    (else
      (cond
        ((and (> logic 1) (< logic 5)) (cache (random 20) (+ 1 (random 4)) (+ 1 (random 10))))
        ...
        ((and (> logic 15) (< logic 20)) (dead_explorer (random 20)))
        ((or (equal? logic 21) (equal? logic 22)) (flaming_fist (+ 2 (random 6)) (+ 1 (random 4))))
        ((and (> logic 27) (< logic 31)) (snake_constrictor))
        ((and (> logic 32) (< logic 36)) (spider (+ 1 (random 6))))
        ((and (> logic 35) (< logic 40)) (statue_ubtao (random 4)))
        ((and (> logic 39) (< logic 44)) (stirge (+ 2 (random 6))))
        ((and (> logic 45) (< logic 49)) (swarm_of_insects (+ 1 (random 4))))
        ((and (> logic 50) (< logic 63)) (ghoul (+ 2 (random 6) (random 6))))
        ((and (> logic 62) (< logic 67)) (skeleton (+ 3 (random 6) (random 6) (random 6))))
        ((and (> logic 66) (< logic 70)) (specter))
        ((and (> logic 69) (< logic 73)) (wight))
        ((and (> logic 72) (< logic 85)) (zombie (+ 1 (random 10)) (+ 3 (random 6) (random 6) (random 6)) (+ 1 (random 4))))
        ...
        ((and (> logic 91) (< logic 96)) (yellow_musk_creeper_and_zombies (+ 3 (random 6) (random 6) (random 6))))
        ((or (equal? logic 96) (equal? logic 97)) (yuan-ti (+ 2 (random 6)) (+ 1 (random 4))))
...
(define (encounters_mountains logic)
  (case logic
    ...
    (else
      (cond
        ((< logic 11) (aarakocra (+ 2 (random 4))))
        ((and (> logic 10) (< logic 17)) (albino_dwarves (+ 4 (random 4))))
        ((and (> logic 16) (< logic 20)) (apes (+ 2 (random 4) (random 4))))
        ((or (equal? logic 20) (equal? logic 21)) (baboons (+ 3 (random 6))))
        ((and (> logic 21) (< logic 25)) (cache (random 20) (+ 1 (random 4)) (+ 1 (random 10))))
        ((or (equal? logic 25) (equal? logic 26)) (chwinga))
        ((or (equal? logic 27) (equal? logic 28)) (cyclops))
        ((and (> logic 28) (< logic 38)) (pteranodon (+ 2 (random 6) (random 6))))
        ((and (> logic 37) (< logic 42)) (quetzalcoatlus (+ 2 (random 4))))
        ((and (> logic 41) (< logic 45)) (dragon))
        ((or (equal? logic 45) (equal? logic 46)) (emerald_enclave (+ 2 (random 4)) (* 5 (+ 2 (random 6) (random 6)))))
        ((and (> logic 46) (< logic 50)) (dead_explorer (random 20)))
        ((and (> logic 49) (< logic 53)) (explorer (+ 1 (random 6)) (random 6)))
        ((and (> logic 52) (< logic 59)) (flying_monkey (+ 3 (random 6) (random 6) (random 6))))
        ((or (equal? logic 59) (equal? logic 60)) (flying_snake (+ 2 (random 6) (random 6))))
        ((or (equal? logic 63) (equal? logic 64)) (giant_wasp (+ 1 (random 6))))
        ((and (> logic 64) (< logic 70)) (girallon))
        ((and (> logic 69) (< logic 73)) (night_hag))
        ((and (> logic 72) (< logic 80)) (pterafolk (+ 3 (random 4))))
        ((and (> logic 80) (< logic 84)) (giant_snake))
        ((and (> logic 83) (< logic 87)) (stirge (+ 2 (random 6))))
        ((and (> logic 86) (< logic 90)) (swarm_of_bats (+ 1 (random 4))))
        ((or (equal? logic 90) (equal? logic 91)) (tabaxi_hunter))
        ((and (> logic 91) (< logic 97)) (troll))
        ((and (> logic 96) (< logic 100)) (wereboar))))))
...
(define (encounters_rivers logic)
  (case logic
    ...
    (else
      (cond
        ((< logic 3) (aarakocra (+ 2 (random 4))))
        ((and (> logic 2) (< logic 7)) (aldani (+ 1 (random 4))))
        ((or (equal? logic 7) (equal? logic 8)) (artus_cimber))
        ((or (equal? logic 10) (equal? logic 11)) (cache (random 20) (+ 1 (random 4)) (+ 1 (random 10))))
        ((and (> logic 11) (< logic 15)) (cannibals (+ 3 (random 6) (random 6) (random 6))))
        ((and (> logic 14) (< logic 18)) (chwinga))
        ((and (> logic 17) (< logic 23)) (crocodiles (+ 2 (random 4))))
        ...
        ((and (> logic 27) (< logic 31)) (plesiosaurus))
        ((and (> logic 30) (< logic 34)) (pteranodon (+ 2 (random 6) (random 6))))
        ((or (equal? logic 34) (equal? logic 35)) (quetzalcoatlus (+ 2 (random 4))))
        ((and (> logic 36) (< logic 40)) (eblis (+ 2 (random 4))))
        ((and (> logic 39) (< logic 43)) (emerald_enclave (+ 2 (random 4)) (* 5 (+ 2 (random 6) (random 6)))))
        ((or (equal? logic 43) (equal? logic 44)) (dead_explorer (random 20)))
        ((and (> logic 44) (< logic 49)) (explorer (+ 1 (random 6)) (random 6)))
  ...
  (case logic
    ...
    (else
      (cond
        ...
        ((and (> logic 2) (< logic 6)) (apes (+ 2 (random 4) (random 4))))
        ((or (equal? logic 6) (equal? logic 7)) (artus_cimber))
        ((and (> logic 7) (< logic 12)) (assassin_vines (+ 1 (random 3))))
        ((or (equal? logic 12) (equal? logic 13)) (baboons (+ 3 (random 6))))
        ((and (> logic 13) (< logic 18)) (cache (random 20) (+ 1 (random 4)) (+ 1 (random 10))))
        ((or (equal? logic 19) (equal? logic 20)) (cyclops))
        ((and (> logic 22) (< logic 26)) (emerald_enclave (+ 2 (random 4)) (* 5 (+ 2 (random 6) (random 6)))))
        ((or (equal? logic 26) (equal? logic 27)) (dead_explorer (random 20)))
        ((and (> logic 27) (< logic 31)) (explorer (+ 1 (random 6)) (random 6)))
        ((or (equal? logic 31) (equal? logic 32)) (flail_snail))
        ((and (> logic 32) (< logic 36)) (flaming_fist (+ 2 (random 6)) (+ 1 (random 4))))
        ...
        ((and (> logic 41) (< logic 45)) (giant_scorpion (+ 1 (random 3))))
        ((and (> logic 44) (< logic 48)) (giant_wasp (+ 1 (random 6))))
        ...
        ((and (> logic 53) (< logic 57)) (kamadan (+ 1 (random 3))))
        ((or (equal? logic 58) (equal? logic 59)) (mad_monkey_mist))
        ((and (> logic 62) (< logic 66)) (giant_snake))
        ((or (equal? logic 66) (equal? logic 67)) (spider (+ 1 (random 6))))
        ((and (> logic 67) (< logic 73)) (statue_ubtao (random 4)))
        ...
        ((and (> logic 80) (< logic 84)) (ghoul (+ 2 (random 6) (random 6))))
        ((and (> logic 83) (< logic 87)) (skeleton (+ 3 (random 6) (random 6) (random 6))))
...
(define (encounters_swamp logic)
  (case logic
    ...
    (else
      (cond
        ((and (> logic -1) (< logic 10)) (aldani (+ 1 (random 4))))
        ((and (> logic 10) (< logic 14)) (assassin_vines (+ 1 (random 3))))
        ((or (equal? logic 14) (equal? logic 15)) (chwinga))
        ((and (> logic 15) (< logic 21)) (crocodiles (+ 2 (random 4))))
        ((or (equal? logic 23) (equal? logic 24)) (brontosaurus))
        ((and (> logic 24) (< logic 30)) (dimetrodon (+ 2 (random 6) (random 6))))
        ((and (> logic 29) (< logic 33)) (hadrosaurus (+ 3 (random 6) (random 6) (random 6)) (+ 1 (random 6))))
        ((or (equal? logic 33) (equal? logic 34)) (pteranodon (+ 2 (random 6) (random 6))))
        ((and (> logic 34) (< logic 39)) (eblis (+ 2 (random 4))))
        ((or (equal? logic 39) (equal? logic 40)) (dead_explorer (random 20)))
        ((and (> logic 40) (< logic 45)) (explorer (+ 1 (random 6)) (random 6)))
        ((or (equal? logic 45) (equal? logic 46)) (flail_snail))
        ((and (> logic 46) (< logic 50)) (flying_snake (+ 2 (random 6) (random 6))))
        ((and (> logic 49) (< logic 53)) (giant_crocodile))
        ((and (> logic 52) (< logic 56)) (giant_frog (+ 2 (random 6) (random 6))))
        ((or (equal? logic 56) (equal? logic 57)) (giant_lizard (+ 1 (random 6))))
        ((or (equal? logic 58) (equal? logic 59)) (giant_snapping_turtle))
        ((or (equal? logic 60) (equal? logic 61)) (giant_wasp (+ 1 (random 6))))
        ...
        ((and (> logic 65) (< logic 69)) (mad_monkey_mist))
        ((and (> logic 71) (< logic 76)) (shambling_mound))
        ((and (> logic 75) (< logic 80)) (snake_constrictor))
        ...
        ((and (> logic 88) (< logic 94)) (swarm_of_insects (+ 1 (random 4))))
...
(define (encounters_wasteland logic)
  (case logic
    ...
    (else
      (cond
        ((and (> logic 0) (< logic 5)) (cache (random 20) (+ 1 (random 4)) (+ 1 (random 10)))) 
        ((and (> logic 4) (< logic 9)) (dragon))
        ((and (> logic 8) (< logic 18)) (dead_explorer (random 20)))
        ((and (> logic 18) (< logic 37)) (firenewt (+ 1 (random 4))))
        ((and (> logic 36) (< logic 45)) (giant_scorpion (+ 1 (random 3)))) 
        ((and (> logic 44) (< logic 54)) (magmin (+ 2 (random 6) (random 6))))
        ((and (> logic 53) (< logic 71)) (mephit (+ 2 (random 6) (random 6)))) 
        ((and (> logic 70) (< logic 78)) (night_hag)) 
        ((and (> logic 78) (< logic 83)) (troll)) 
        ((or (equal? logic 83) (equal? logic 84)) (ghoul (+ 2 (random 6) (random 6)))) 
        ((and (> logic 84) (< logic 95)) (skeleton (+ 3 (random 6) (random 6) (random 6)))) 
...
;; (+ 1 (random 3)) 
(define (allosaurus number_of_creatures)
...
(define (giant_scorpion number_of_creatures)
    ...
    shivering sickness (see “Diseases, DMG page 40).
...
(define (statue_ubtao type)
    ...
    tribute by some jungle creature.\n") (treasure (random 100)))
        ((1) (display "Goblin, grung, and su-monster skulls are piled around
    the statue’s base.\n"))
