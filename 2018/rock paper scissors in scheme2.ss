...
(define (start counter user_score computer_score tied_games)
  ...
  (cond
    ...
    (else (display "Pick your poison. \n (rock, paper or scissors):") 
      (update_end_scores (start_game) counter user_score computer_score tied_games) )))


;; update_end_scores(symbol x, int counter, int user_score, int computer_score, int tied_games)
;;
;; increments counter and the appropriate value based on return value from function compair_choice()

(define (update_end_scores x counter user_score computer_score tied_games)
  (cond
    ((equal? x 'u) (start (+ counter 1) (+ user_score 1) computer_score tied_games ))
    ((equal? x 'c) (start (+ counter 1) user_score (+ computer_score 1) tied_games ))
    ((equal? x 't) (start (+ counter 1) user_score computer_score (+ tied_games 1)))))
