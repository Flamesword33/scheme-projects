...
;; by Nathan Pelletier
;;
;; Starts a game of rock paper scissors with the user that runs for ten games.
;; 
;;
;;;;;;;;;;;;;
;; METHODS ;;
...
;;   print the users win tally      GOT IT TO RETURN THE INFORMATION BUT CANT USE IT
;;   print the pc's win tally       GOT IT TO RETURN THE INFORMATION BUT CANT USE IT
;;   print the number of tied games GOT IT TO RETURN THE INFORMATION BUT CANT USE IT
...
(define (start counter user_score computer_score tied_games)
  ...
  (cond
    ((>= counter 10) (display "\nGame Over") (score_display user_score computer_score tied_games))
    (else (display "Pick your poison. \n (rock, paper or scissors):") 
;      (start_game) (start (+ counter 1) user_score computer_score tied_games))))
      (returned_letter(start_game)) 
      (cond
        ((equal? (returned_letter) 'u) (start (+ counter 1) (+ user_score 1) computer_score tied_games ))
        ((equal? (returned_letter) 'c) (start (+ counter 1) user_score (+ computer_score 1) tied_games ))
        ((equal? (returned_letter) 't) (start (+ counter 1) user_score computer_score (+ tied_games 1)))))))

(define (returned_letter x) x)
...
(define (score_display user_score computer_score tied_games)
  (display "\nYou won: ") (display user_score) (display " times. \n You lost: ") 
  (display computer_score) (display " times. \n The game was tied: ") (display tied_games))  
...
(define (start_game)
 (cond (#t (compair_choices (get_user_choice(readLine)) (computer_choice)) )))
...
(define (get_user_choice user_input) 
  (cond
    ...
    ((equal? (car user_input) '#\S) 's)
    (else (display "Incorrect input. Try again. \n (rock, paper, scissors): ") 
...
(define (compair_choices user computer)
  (cond
    ...
    (else (display "Something went wrong with compair_choices" ) )));compair_choices
